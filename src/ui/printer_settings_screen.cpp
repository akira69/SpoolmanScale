...
namespace {
enum class Page { MAIN, SIZES, CUSTOM };
...
lv_obj_t* device_rows[8]{};
lv_obj_t* device_names[8]{};
...
uint16_t custom_width = M220_DEFAULT_MEDIA_WIDTH_MM;
uint16_t custom_length = M220_DEFAULT_MEDIA_LENGTH_MM;

uint16_t savedDimension(const char* key, uint16_t fallback,
                        uint16_t minimum, uint16_t maximum) {
  const int value = prefsGetInt(key, fallback);
  return value >= minimum && value <= maximum ? value : fallback;
}

uint16_t savedWidth() {
  return savedDimension("m220_media_w", M220_DEFAULT_MEDIA_WIDTH_MM, 20, 75);
}

uint16_t savedLength() {
  return savedDimension("m220_media_h", M220_DEFAULT_MEDIA_LENGTH_MM, 10, 150);
}

void saveMedia(uint16_t width, uint16_t length) {
  prefsPutInt("m220_media_w", width);
  prefsPutInt("m220_media_h", length);
}
...
void clearPointers() {
  ...
  for (size_t i = 0; i < 8; ++i) device_rows[i] = device_names[i] = nullptr;
}
...
void refreshDeviceSelection() {
  const String saved = prefsGetString("m220_addr");
  for (size_t i = 0; i < device_count; ++i) {
    if (!device_rows[i] || !device_names[i]) continue;
    const bool selected = !saved.isEmpty() && saved == devices[i].address;
    styleListRow(device_rows[i], selected);
    if (selected) lv_label_set_text_fmt(device_names[i], "%s  %s", LV_SYMBOL_OK, devices[i].name);
    else lv_label_set_text(device_names[i], devices[i].name);
...
void updateSavedAddress() {
  ...
  const String address = prefsGetString("m220_addr");
  if (address.isEmpty()) {
    ...
    const String name = prefsGetString("m220_name");
    lv_label_set_text(selected_value, name.isEmpty() ? address.c_str() : name.c_str());
    lv_label_set_text(selected_address, name.isEmpty() ? "" : address.c_str());
...
void renderDevices() {
  ...
  for (size_t i = 0; i < 8; ++i) device_rows[i] = device_names[i] = nullptr;
  if (!device_count) {
    showListMessage(T(STR_LABEL_M220_NONE));
    return;
  }
  for (size_t i = 0; i < device_count && lvPoolHasRoomForRow(); ++i) {
    ...
    lv_obj_add_event_cb(row, [](lv_event_t* event) {
      ...
      for (size_t i = 0; i < device_count; ++i)
        if (!strcmp(address, devices[i].address)) prefsPutString("m220_name", devices[i].name);
      prefsPutString("m220_addr", address);
      updateSavedAddress();
    }, LV_EVENT_CLICKED, devices[i].address);

    device_names[i] = addText(row, devices[i].name, UI_FONT_BODY, UI_COL_INK_2);
...
void buildMainScreen() {
  ...
  lv_obj_set_width(selected_value, 320);
  ...
  lv_obj_t* available = addText(devices_panel, T(STR_PRINTER_AVAILABLE), UI_FONT_BODY,
                                UI_COL_INK_2);
  ...
  snprintf(media_text, sizeof(media_text), "%u x %u mm",
           unsigned(savedWidth()), unsigned(savedLength()));
...
void buildSizeScreen() {
  ...
  const uint16_t width = savedWidth();
  const uint16_t length = savedLength();
  for (const auto& size : kMediaSizes) {
    char text[20];
    ...
    addSizeRow(text, size.width == width && size.length == length,
      [](lv_event_t* event) {
        ...
        saveMedia(size->width, size->length);
        main_pending = true;
...
void adjustCustom(lv_event_t* event) {
  ...
  const uint16_t minimum = adjustment->width ? 20 : 10;
  const uint16_t maximum = adjustment->width ? 75 : 150;
...
void addAdjustmentRow(int y, StringID title, bool width) {
  ...
  for (int side = 0; side < 2; ++side) {
    ...
    lv_obj_add_event_cb(button, adjustCustom, LV_EVENT_CLICKED,
                        const_cast<Adjustment*>(&kAdjustments[config + side]));
...
void buildCustomScreen() {
  custom_width = savedWidth();
  custom_length = savedLength();
  ...
  lv_obj_add_event_cb(save, [](lv_event_t*) {
    saveMedia(custom_width, custom_length);
    main_pending = true;
  }, LV_EVENT_CLICKED, nullptr);
...
void handlePrinterSettingsDeferredActions() {
  ...
  if (back_pending) {
    ...
    else if (page == Page::SIZES) buildMainScreen();
    else {
  ...
  if (scan_pending && screen && page == Page::MAIN) {
    scan_pending = false;
    showListMessage(T(STR_PRINTER_SCANNING));
    lv_refr_now(nullptr);
    device_count = phomemoM220Scan(devices, 8);
    renderDevices();
    updateSavedAddress();
  }
}
...
void hidePrinterSettingsOverlays() {
  ...
  sizes_pending = custom_pending = main_pending = false;
}
