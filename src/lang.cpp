...
const char* const STRINGS[][3] = {
  ...
  { "Trocknungserinnerung", "Drying Reminder",
    "Rappel de séchage" },  // STR_BTN_DRYING_REMINDER
  { "Trocknungserinnerung", "Drying Reminder",
    "Rappel de séchage" },  // STR_DRYING_REMINDER_TITLE
  ...
  { "FilaMan legt beides an verschiedenen Stellen an und zeigt es jeweils nur einmal.<br><b>API-Key:</b> auf das Zahnrad neben dem Benutzernamen klicken, <b>API Keys</b> öffnen und einen Key mit <b>spools:read</b> erstellen. Oben eintragen und speichern. Für Label-Anfragen muss ein PC-Tab mit demselben Benutzer angemeldet sein.<br><b>Gerätecode:</b> <b>Admin-Bereich &gt; Geräte</b> öffnen und <b>Gerät erstellen</b> klicken. Den sechsstelligen Code oben eintragen und registrieren.<br>Ein Gerät anlegen kann nur ein Admin. Den Key mit demselben Konto erstellen, sonst lehnt FilaMan die AMS-Zuordnung ab.",
    "FilaMan creates the two in different places and shows each only once.<br><b>API key:</b> click the gear icon next to your user name, open <b>API Keys</b> and create a key with <b>spools:read</b>. Enter it above and save. Keep a PC tab signed in as the same user for label requests.<br><b>Device code:</b> open <b>Admin Panel &gt; Devices</b> and click <b>Create Device</b>. Enter the six character code above and register.<br>Only an admin can create a device. Create the key with the same account, or FilaMan refuses the AMS assignment.",
    "FilaMan crée la clé et le code à deux endroits différents, et n'affiche chacun qu'une seule fois.<br><b>Clé d'API :</b> cliquez sur l'icône d'engrenage à côté de votre nom d'utilisateur, ouvrez <b>Clés API</b> (API Keys) et créez une clé avec <b>spools:read</b>. Saisissez-la ci-dessus et cliquez sur Enregistrer. Pour les demandes d'étiquette, gardez un onglet PC connecté avec le même utilisateur.<br><b>Code d'appareil :</b> ouvrez <b>Espace admin &gt; Appareils</b> (Admin Panel > Devices) et cliquez sur <b>Créer un appareil</b> (Create Device). Saisissez le code à six caractères ci-dessus, puis cliquez sur Associer l'appareil.<br>Seul un administrateur peut créer un appareil. Créez la clé avec ce même compte, sinon FilaMan refuse l'affectation AMS." },  // STR_W_FM_SETUP
