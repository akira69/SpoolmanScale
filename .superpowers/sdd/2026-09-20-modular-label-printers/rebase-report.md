...
| `0b591ff` Add NFC-free FilaMan spool printing setup | `src/app/app_loop.cpp`, `src/lang.cpp`, `src/ui/spoolman_lookup.cpp` | Kept upstream `newly_placed` detection and wake rules while adding feature header-status refreshes. Final audit restored the feature's existing Bambu `TagSeen::note` block. Added spool/printer strings without losing French. Retained upstream server-color helper and lookup logic while changing `querySpoolmanById` to return false on failure and true on success for manual loading. |
...
- `app_loop.cpp`: retained upstream flash-log maintenance, server-reach popup after deferred handlers, restored-reach notification and shared failure-aware `paintTagStatus`. Feature label/manual-spool/printer handlers, pending-NFC-clear cancellation, tag header updates and pre-existing tag recording remain.
...
After the integration correction commit containing this report, `upstream/dev...HEAD` is **0 upstream-only, 42 local-only**. Relative to the existing local remote-tracking `origin/feat/filaman-label-integration` (`e1da91cec73744aa5427f0c7369a4fcbb1b11367`), divergence is **29 origin-only, 45 local-only**. Origin was not fetched or pushed for this task. The backup branch remains at the original tip.
