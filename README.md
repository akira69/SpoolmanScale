...
To print without NFC, first open **Settings → Connection → Printer**. Choose the printer model separately from its BLE device: M220 is the default and hardware-verified model; M110 is **Experimental**, with a printable width of at most 48 mm pending external hardware validation. Scans list up to 24 captured BLE devices and only prioritize likely printer names. Choose the loaded label size as width across the printer × feed length; no printer is selected by default, and **Settings** shows **Print label** only after a printer is configured.

Then, back in **Settings**, tap **Print label**. The spool list loads the 10 newest active spools per page. **Search** accepts descriptive text or `#ID`; **Previous** and **Next** retain the query, and **Clear** restores the newest list. Select a spool and tap **Print label** in its details. This opens the preview with the preset selected by the configured FilaMan API-key user; **Change preset** updates that shared choice. If FilaMan is offline or does not support shared selection, the scale falls back to its last NVS choice. The preview also provides **Open on PC** and **Print**; **Open on PC** lets FilaMan print or export PDF. Presets stay flat at 10 or fewer and group alphabetically above 10. Every blocking spool, scan, preset, preview, PC, and BLE action shows a loading overlay. See the [SpoolmanScale repository](https://github.com/Niko11111/SpoolmanScale), [documentation](https://niko11111.github.io/SpoolmanScale-Docs/), and [FilaMan](https://github.com/Fire-Devils/filaman-system) API documentation.
