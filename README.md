...
To print without NFC, first open **Settings → Connection → Printer**. Choose the printer model separately from its BLE device: M220 is the default and hardware-verified model; M110 is **Experimental**, with a printable width of at most 48 mm pending external hardware validation. Scans list every captured BLE device and only prioritize likely printer names. Choose the loaded label size as width across the printer × feed length; no printer is selected by default, and **Settings** shows **Print label** only after a printer is configured.

Then tap **Print label**, choose a saved preset or the default, and open the preview. It provides **Open on PC**, **Print**, and **Change preset**; **Open on PC** lets FilaMan print or export PDF. Presets stay flat at 10 or fewer and group alphabetically above 10. Every blocking spool, scan, preset, preview, PC, and BLE action shows a loading overlay. See the [SpoolmanScale repository](https://github.com/Niko11111/SpoolmanScale), [documentation](https://niko11111.github.io/SpoolmanScale-Docs/), and [FilaMan](https://github.com/Fire-Devils/filaman-system) API documentation.
